...
8. Download the soil compaction data to your `data` folder.
9. Do all of your set up (reading in packages/data) at the top of your document.
  ...
  Store the soil compaction data as an object called `soil_compaction`.  
...
You are working at a restoration site where you are managing planting along a heavily trafficked pedestrian trail. You notice that the plants you put in the ground aren't doing well, and you wonder if the soil compaction around the trail is related to plant growth.  

You decide to measure plant growth using root depth (in mm) and [soil compaction as force to penetrate](https://extension.sdstate.edu/soil-penetration-resistance-soil-health-indicator) (in Megapascals, or MPa). You measure the roots of **insert number here** individual **insert species here.**  

#### a. An appropriate test

In 1-3 sentences, name the appropriate test(s) to determine the strength of the relationship between soil compaction and root depth (hint: there are two). Describe the differences between the two tests.  
...
Double check your understanding of the appropriate test. You are not trying to predict root depth using compaction, but trying to understand the strength of the relationship between the two variables.
:::
...
Create a visualization that would be appropriate for showing the relationship between soil compaction (force to penetrate, MPa) and **insert species here** root depth (mm).  
...
In the order that is appropriate:  
- check your assumptions  
- run your test  

Be sure each section is separated using headers (e.g. you should have a header for "checking assumptions" and another for "running the test").  
...
- what assumptions you had to check, and how  
...
You're working on a team of people at this restoration site who are also concerned about plant growth. In 1-3 sentences, write what you would communicate to them about the results of this test and what you would recommend for actions your team could take to improve plant growth (as it relates to soil compaction) at your site.  

Be cognizant of your audience as you are writing: what would they need to know to take action? What parts are not relevant?  
...
For example, if you are making a sculpture, you could draw your form and create a draft out of clay, paper, etc. but not start on the final piece.  
...
_You will only earn a token for workshop attendance week 9 if you have a draft to share for peer review. No exceptions._
:::
...
- [ ] have an informative README with the following sections filled out: General information, Data and file information, and Rendered output. See the repo you created for workshop 7 and workshop 8 for a reference.  
