...
set.seed(1)
urchins <- tibble(
  mpa = rnorm(n = 60, mean = 18, sd = 5),
  non_mpa = rnorm(n = 60, mean = 14, sd = 10)
) |> 
  pivot_longer(cols = mpa:non_mpa,
               names_to = "type",
               values_to = "diameter_cm")

t.test(diameter_cm ~ type,
       data = urchins)
```


You're a fisheries manager interested in the effects of marine protected areas on the size of red urchins (_Mesocentrotus franciscanus_). You find a study in which researchers examined exactly this effect. In their results section, the researchers wrote:  

> We found a difference in red urchin diameter (cm) between marine protected areas and 
> non-protected areas (Welch's two-sample t-test, t(85.1) = 3.39, p = 0.001, $\alpha$ = 0.05).
...
> t(85.1) = 3.39, p = 0.001, $\alpha$ = 0.05
...
> Although tussock density was lower at the burned site... average tussock mound diameters were larger by 4 cm (Table 1; p < 0.001).
...
Find Table 1 (as referenced in the paper). In 1-2 sentences, summarize the  
- means and SEs as the authors calculated them and  
- your interpretation of the difference between groups (i.e. why is it)

#### c. Cleaning and organizing data

:::{.callout-note collapse="true" title="Response variable column in original dataset"}
Use the "Average diameter" column for tussock height.
:::
...
3. selecting only the site and diameter column  
...
#### e. Tests and effect sizes 

Use a Welch's t-test to compare mean tussock diameter (cm) between groups.  
...
## Problem 4. Cleaning, wrangling, and visualization 
