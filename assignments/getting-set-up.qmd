...
[Optional check in due on Wednesday January 7 (Week 1) at 11:59 PM]{style="color: #79ACBD; font-size: 24px;"}  

In this class, we’ll be using R and RStudio to code up our statistical analyses. Walk through these steps to make sure you have both programs on your computer and that everything is working properly. You need to make sure these tasks are completed before workshop on Friday.   

:::{.callout-important title="Do these set up steps as soon as possible!"}
At minimum, you need to have read through and completed tasks 1-7 (modifications for Jupyter instance below). If you want to submit the optional check-in for An to verify that you've done things correctly, you can do task 8 and submit your screenshots on Canvas.  
...
If you are using the virtual machine, then you only need to do the following tasks (skip the others):    

- Task 3. Open RStudio  
- Task 4. Change your settings  
- Task 6. Test out reading in a package  
- Task 7. Set up a folder for class materials  
- (optional) Task 8. Take a screenshot of your set up  

See the resource on [using the virtual machine](https://winter-2026.envs-193ds.com/resources/using-virtual-machine) for more details.
:::
...
An will be using R version 4.5.2 and RStudio version 2025.09.2+418.
:::
...
:::{.callout-note title="Jupyter instance steps"}
If you're using the Jupyter instance, follow the directions in the ["using the virtual machine" resource](https://winter-2026.envs-193ds.com/resources/using-virtual-machine).
:::

# Task 8. Take a screenshot of your set up.

Take a screenshot of your RStudio window with the code for `install.packages("tidyverse")` and `library(tidyverse)` in your Console and submit it to the portal on Canvas. Your screenshot should look something like this:
...
Then, take a screenshot of your 'ENVS-193DS' folder set up. It should be evident what the file path is, either from a path at the bottom of the window or on the side. It doesn't matter where the file path is, just as long as you can tell where the folder is. Your screenshot should look something like this: 

![](/assignments/images/getting-set-up/file-path.png){fig-align="center" width="90%"}

:::{.callout-note title="Making sure your file path is displayed"}
...
**Upload both screenshots to Canvas for your optional check-in.**
