...
date: 2026-03-31
categories: [central tendency, spread, regression, histogram, jitterplot, box-and-whisker plot, bar plot, probability mass, probability density, probability distribution, normal distribution, z-score calculation, t-distribution, uniform distribution, binomial distribution, poisson distribution]
citation:
  url: https://spring-2026.envs-193ds.com/lecture/lecture_week-01.html
---
...
## 6. jitter plot and box plot example

```{r jitterplot}
set.seed(666)

pretend_diameters <- cbind(
  Starburst = rnorm(40, mean = 12, sd = 4), 
  `Giant green` = rnorm(40, mean = 17, sd = 5), 
  Moonglow = rnorm(40, mean = 8, sd = 2)
) %>% 
  as_tibble() %>% 
...
ggplot(pretend_diameters, aes(x = name, y = value, color = name)) +
...
ggplot(pretend_diameters, aes(x = name, y = value, fill = name)) +
  geom_jitter(width = 0.1, height = 0, alpha = 0.8, size = 2) +
...
ggplot(pretend_diameters, aes(x = name, y = value)) +
  geom_jitter(height = 0, width = 0.1, alpha = 0.4, shape = 21) +
  geom_boxplot(alpha = 0.8, aes(fill = name)) +
