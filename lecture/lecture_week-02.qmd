...
set.seed(7)
# snails <- rt(n = 41, df = 40, ncp = 2.5) 
snails <- rnorm(n = 20, mean = 1.25, sd = 0.19)
```
...
#| fig-height: 6
#| fig-align: center
breakpoints <- round(seq(from = min(snails), to = max(snails), length.out = 7), 2)
...
hist <- enframe(snails) %>% 
  ...
  theme_classic() +
  labs(x = "Shell diameter (mm)", y = "Count")
hist
```
...
enframe(snails) %>% 
  ...
  theme_classic() +
  labs(x = "Shell diameter (mm)", y = "Density")
```
...
snail_conflev <- tribble(
  ...
  mutate(upper = case_when(
    conflev == 0.85 ~ mean(snails) + qt(p = 0.15/2, df = 19)*sd(snails)/sqrt(length(snails)),
    conflev == 0.90 ~ mean(snails) + qt(p = 0.1/2, df = 19)*sd(snails)/sqrt(length(snails)),
    conflev == 0.95 ~ mean(snails) + qt(p = 0.05/2, df = 19)*sd(snails)/sqrt(length(snails)),
    conflev == 0.99 ~ mean(snails) + qt(p = 0.01/2, df = 19)*sd(snails)/sqrt(length(snails))
  ),
  lower = case_when(
    conflev == 0.85 ~ mean(snails) - qt(p = 0.15/2, df = 19)*sd(snails)/sqrt(length(snails)),
    conflev == 0.90 ~ mean(snails) - qt(p = 0.1/2, df = 19)*sd(snails)/sqrt(length(snails)),
  ...
  mutate(mean = mean(snails))
  # se <- s/sqrt(n)

ggplot() +
  geom_point(data = enframe(snails), aes(x = 0.84, y = snails),
             alpha = 0.6, shape = 21) +
  geom_point(data = enframe(snails), aes(x = 0.89, y = snails),
             alpha = 0.6, shape = 21) +
  geom_point(data = enframe(snails), aes(x = 0.94, y = snails),
             alpha = 0.6, shape = 21) +
  geom_point(data = enframe(snails), aes(x = 0.98, y = snails),
             alpha = 0.6, shape = 21) +
