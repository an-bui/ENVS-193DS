...
citation:
  url: https://spring-2026.envs-193ds.com/lecture/lecture_week-09.html
---
...
ggplot(data = drought_exp_clean,
       mapping = aes(x = leaf_dry_weight_g,
                     y = sla)) +
  geom_point()
...
# frog toxicity interaction terms
...
# bighorn sheep example

length = 56.1 + 0.41 × Age + 5.6 × DesertBighorn
...
sheep <- tibble(
  ...
  length = rnorm(n = 30, mean = 56.1, sd = 3) + 1.2*age) |> 
  mutate(curl = case_when(
    length > mean(length) ~ "curl",
    length < mean(length) ~ "no curl"
  ),
  length = case_when(
    subspecies == "DesertBighorn"~ length + 5.6,
    .default = length
  ),
  length = case_when(
    curl == "curl" ~ length + 10.5,
    curl == "no curl" ~ length
  ))
  

mod <- lm(
  length ~ age + subspecies + curl,
  data = sheep
)
