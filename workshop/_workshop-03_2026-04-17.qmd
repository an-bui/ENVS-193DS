...
```{r histogram}
ggplot(data = maples_2003, # starting data frame
       mapping = aes(x = stem_length)) + # x-axis (no y-axis for histogram)
...
ggplot(data = maples_2003, # starting data frame
       mapping = aes(sample = stem_length)) + # y-axis for QQ plot (no x-axis for QQ plot)
...
**We determined that group variances were equal (F-test of equal variances, F(119, 119) = 1.06, p = 0.76).**    
...
maples_ci <- maples_2003 |> # start with the maples_2003 data frame
  group_by(watershed) |> # group by production type
  summarize(mean = mean(stem_length), # calculate the mean
            n = length(stem_length), # count the number of observations
            df = n - 1, # calculate the degrees of freedom
            sd = sd(stem_length), # calculate the standard deviation
            ...
            ci_upper = mean + margin # calculate the upper bound of the confidence interval
          ) |> 
...
ggplot(data = maples_2003, # using the maples_2003 data frame
  ...
  geom_pointrange(data = maples_ci, # using the maples_ci data frame
                  aes(x = watershed, # x-axis
                      y = y, # y-axis
                      ymax = ci_lower, # upper bound of confidence interval
                      ymin = ci_upper)) + # lower bound of confidence interval
