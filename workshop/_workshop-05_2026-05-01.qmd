...
# base layer: ggplot
ggplot(data = pie_crab_clean, 
       mapping = aes(sample = size)) + # y-axis
...
```{r crab-tukey}
TukeyHSD(crab_anova) # running Tukey HSD
```
...
```{r crab-effectsize}
eta_squared(crab_anova) # calculating effect size
```
