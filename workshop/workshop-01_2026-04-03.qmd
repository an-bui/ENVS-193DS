...
Let's say we want to figure out what the median number of visits is for Joshua Tree is across all years and months.  
...
Then, you can calculate the median monthly visits for Joshua Tree across all years using the `summarize()` and `mean()` functions.

```{r}
df2 <- summarize(df1, # use the df1 data frame
                 median_visits = median(visits)) # calculate the mean number of visits
```
...
median_jtnp_visits <- parks_visits |> # start with the `parks_visits` data frame
  filter(park == "jtnp") |> # filter to only include Joshua tree
  summarize(median_visits = median(visits)) # calculate the mean number of visits at Joshua Tree
```
...
You should see the median number of visits to Joshua Tree across all years and all months pop up.  
...
median_jtnp_visits_2024 <- parks_visits |> # start with parks_visits object
  filter(park == "jtnp" & year == "2024") |> # filter to only include jtnp and 2024
  summarize(median_visits = median(visits)) # calculate mean number of visits
```

What's the mean number of visits to Joshua Tree in 2024 across all months?

```{r mean-jtnp-2024-display}
median_jtnp_visits_2024
```
...
2. the "mapping" (using the aesthetics function called `aes()`): you have the tell R the data frame name (in this case, `parks_visits`) and what the x- and y-axes should be   
...
ggplot(data = parks_visits, # using the data frame parks_visits
       mapping = aes(x = park, # 2. naming the aesthetics: the x-axis should be the 3 parks
...
parks_summary <- parks_visits |> # start with the parks_visits data frame
  ...
  ungroup() # ungrouping the data frame
...
ggplot(data = annual_visits, # use the annual_visits data frame
       mapping = aes(x = year, # the x-axis should be year
